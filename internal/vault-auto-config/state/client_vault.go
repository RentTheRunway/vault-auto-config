...
var readOnlyPaths = make(map[string]bool)

func init() {
	readOnlyPaths["sys/auth/token"] = true
	readOnlyPaths["sys/policy/default"] = true
	readOnlyPaths["sys/policy/root"] = true
}
...
func (c *VaultClient) Read(path string, args ...interface{}) (Payload, error) {
	data, err := c.client.Logical().Read(fmt.Sprintf(path, args...))
